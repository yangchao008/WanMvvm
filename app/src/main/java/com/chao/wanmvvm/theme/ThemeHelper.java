...
public class ThemeHelper {
    ...
    public static void setTheme(List<Pair<TextView,Object>> list){
        if (list != null && !list.isEmpty()){
            for (Pair pair : list){
                TextView first = (TextView) pair.getFirst();
                Object second = pair.getSecond();
                if (first != null){
                    if (second != null && second instanceof TextSizeHelper.TextSizeLevel){
                        first.setTextSize(TextSizeHelper.INSTANCE
                                .getTextSize((TextSizeHelper.TextSizeLevel) second));
                    }else if (second != null && second instanceof TextColorHelper.TextColorLevel){
                        first.setTextColor(TextColorHelper.INSTANCE
                                .getTextColor((TextColorHelper.TextColorLevel) second));
                    }else if (second != null && second instanceof TextFontHelper.TextFontLevel){
                        first.setTypeface(TextFontHelper.INSTANCE
                                .getTextFont((TextFontHelper.TextFontLevel) second));
                    }
                }
            }
        }
    ...
    public static void setTheme(Pair<TextView,Object> pair){
        if (pair != null){
            List<Pair<TextView,Object>> list = new ArrayList<>();
            list.add(pair);
            setTheme(list);
        }
    }

//    public static List<Pair<TextView,Object>> addPair(TextView textView,TextSizeHelper.TextSizeLevel textSizeLevel,
//                                                      TextColorHelper.TitleColorLevel titleColorLevel){
//        List<Pair<TextView,Object>> list = new ArrayList<>();
//        list.add(textView,textSizeLevel);
//        list.add(textView,titleColorLevel);
//        return list;
//    }

    public static TextView setTextColorAndSize(TextView textView,TextColorHelper.TextColorLevel textColorLevel,
                                               TextSizeHelper.TextSizeLevel textSizeLevel){
        if (textView != null){
            List<Pair<TextView,Object>> list = new ArrayList<>();
            Pair pair = new Pair(textView,textColorLevel);
            Pair pair2 = new Pair(textView,textSizeLevel);
            list.add(pair);
            list.add(pair2);
            setTheme(list);
        }
    ...
    public static TextView setTextColor(TextView textView,TextColorHelper.TextColorLevel textColorLevel){
        if (textView != null){
            Pair pair = new Pair(textView,textColorLevel);
            setTheme(pair);
        }
    ...
    public static TextView setTextSize(TextView textView,TextSizeHelper.TextSizeLevel textSizeLevel){
        if (textView != null){
            Pair pair = new Pair(textView,textSizeLevel);
            setTheme(pair);
        }
        return textView;
    }
